 ...
 *   用 --intraday：它**自动**关闭推送并另存 eod-<日>-intraday.json，
...
/**
 * 盘中模式（--intraday）是滚动窗口近似口径，不是当日正式尾盘结果。
 * 推送链路仍然强制关闭（近似值不值得打扰微信/邮箱）；
 * D1 自 2026-09-22 起放开入库（mode='intraday'，独立口径行，不会与 formal/observe 混淆），
 * 报告查看页 /api/tail/runs 才能直接列出盘中版报告，不再靠前端探测文件是否存在。
 * 显式 `--no-d1` 仍可临时关掉落库。
 */
if (OPT.intraday) {
  OPT.noNotify = true;
