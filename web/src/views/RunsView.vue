...
/* 窄屏（≤820px）：表格翻转成紧凑卡片——6 列网格按语义分行（身份 / 档位规则 / 板块 / 指标×2 / 理由） */
@media (max-width: 820px) {
  ...
  .grid td { display: block; width: auto; padding: 0 2px; border: none; text-align: left; white-space: normal; }
  .grid td::before {
  ...
  .grid td[data-label="档位"] { grid-column: 1 / 3; }
  .grid td[data-label="规则"] { grid-column: 3 / 7; }
  .grid td.sector { grid-column: 1 / 7; }
  /* 指标格：价格/R01/换手一行，量比/市值/板块强度一行 */
  .grid td.num { grid-column: span 2; }
  .grid td.reason { grid-column: 1 / 7; color: var(--muted); }
